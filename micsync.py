...
class Rsync:
    ...
    def shallModifyExisting(srcsLst, dst, suspendPrintDirs):
        outpLines = Rsync._run(["-n", "-a", "-h", "-P", "--existing"] + Rsync._pathsForRsync(srcsLst, dst),
                               suspendPrintDirs, suspendPrintDirs, dst)
        if (outpLines):
    ...
    def shallDeleteInDst(srcsLst, dst, suspendPrintDirs):
        outpLines = Rsync._run(
            ["-n", "-a", "-h", "-P", "--delete", "--ignore-existing", "--existing"] + Rsync._pathsForRsync(srcsLst,
                                                                                                           dst),
            suspendPrintDirs, True, dst)
        if (outpLines):
...
class Flags:
    ...
    def getRsyncOptions(self, dst, srcs):
        if (self.askForModified):
            optNoModify = Rsync.NO_OPTIONS if Rsync.shallModifyExisting(srcs, dst,
                                                                        self.suspendPrintDirs) else Rsync.NO_MODIFY
        else:
        ...
        if (self.dontAskForDeleted):
            ...
            if (self.allowDeleting):
                optDelete = Rsync.DELETE if Rsync.shallDeleteInDst(srcs, dst,
                                                                   self.suspendPrintDirs) else Rsync.NO_OPTIONS
            else:
